...
3.	Copy the files from `vpc-ha-iac/multi-region` folder to the project folder that you created in the Terraform installation directory.

4.  Use the instructions defined in the `example.userinput.auto.tfvars` file to rename the file and edit the variables to set the values for your configuration.
...
**KT Question: the Folder is "instance" not instances, and there are no images in the file, that I can see. Should this be the instance_variables.tf file in the main folder?**
...
**KT NOTE:  Do the users need to edit any variable files for this set of scripts?  Are there any more in to add to this list? Are the changes made in the different variables files in the main folder?** 
